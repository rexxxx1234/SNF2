...
X_embedded = TSNE(n_components=2, metric='precomputed').fit_transform(2 - final_result.values)
#X_embedded = tsne_p(Wall_final, no_dims=2)
plt.scatter(X_embedded[:, 0], X_embedded[:, 1], c=alllabels, s=1.5, cmap='Spectral')
plt.title('t-SNE visualization of union 1332 butterfly after batch correction')
save_path = op.join(testdata_dir, 't-SNE_1332_really.png')
