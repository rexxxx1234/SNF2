...
def data_indexing(matrices):
    ...
    for i in range(0, len(original_order)):
        for j in range(i+1, len(original_order)):
            ...
            dict_commonSample.update(dict_commonSample.fromkeys([(i, j), (j, i)], commonList))
            dict_uniqueSample[(i, j)] = list(set(original_order[i]).symmetric_difference(commonList))
            dict_uniqueSample[(j, i)] = list(set(original_order[j]).symmetric_difference(commonList))

    return dict_commonSample, dict_uniqueSample, original_order